<template>
  <div class="full-width full-height">
    <div class="layout-wrap row items-center justify-center">
      <div v-for="item in list" class="row justify-center col-4">
...
export default {
  data () {
    return {
      list: [
        { name: this.$mg_t('title.title2'), icon: 'dvr', url: `/layout` },
        { name: this.$mg_t('title.title1'), icon: 'dvr', url: `/importPage` },
        { name: this.$mg_t('title.title3'), icon: 'dvr', url: `/showpage` }
      ]
    }
  },
  created () {
  ...
  methods: mapMutations(`${process.env.APP_SCOPE_NAME}`, ['setPageTitle'])
}
</script>
